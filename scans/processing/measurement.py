...
def get_head_depth_from_side_image(side_image_path: str) -> float:
    try:
        ...
        print(f"Warning: Could not calculate head depth from side image: {e}")
        return 0.0
...
def get_dynamic_2d_measurements(front_image_path: str, side_image_path: str) -> Optional[Dict[str, float]]:
    try:
        ...
        with mp_face_mesh.FaceMesh(
            ...
            if image is None:
                raise FileNotFoundError(f"Image not found at path: {front_image_path}")
            
            results = face_mesh.process(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
            
            if not results.multi_face_landmarks:
                print(f"WARNING: No face landmarks detected in image: {front_image_path}. Using default measurements.")
                return {'head_width': 15.0, 'head_length': 20.0, 'ear_to_ear': 15.0, 'eye_to_eye': ASSUMED_IPD_CM, 'head_height': 20.0, 'head_circumference_A': 55.0, 'forehead_to_back_B': 20.0, 'cross_measurement_C': 15.0, 'under_chin_D': 10.0, 'eyebrow_to_earlobe_E': 8.0, 'eye_corner_to_ear_F': 7.0, 'ear_height_G': 6.5, 'ear_width_H': 3.5, 'cheek_guard_clearance_L': 2.0, 'cheek_guard_height_M': 5.0, 'cheek_guard_width_N': 4.0}
            ...
            ipd_px = calculate_pixel_distance(left_pupil, right_pupil, image_w, image_h)

            left_ear = face_landmarks.landmark[LEFT_EAR_TRAGUS_INDEX]
            right_ear = face_landmarks.landmark[RIGHT_EAR_TRAGUS_INDEX]
            face_width_px = calculate_pixel_distance(left_ear, right_ear, image_w, image_h)

            AVG_FACE_WIDTH_TO_IPD_RATIO = 2.25
            subject_pixel_ratio = face_width_px / ipd_px if ipd_px > 0 else AVG_FACE_WIDTH_TO_IPD_RATIO
            correction_factor = subject_pixel_ratio / AVG_FACE_WIDTH_TO_IPD_RATIO
            corrected_assumed_ipd_cm = ASSUMED_IPD_CM * correction_factor
            
            pixel_to_cm_ratio = corrected_assumed_ipd_cm / ipd_px if ipd_px > 0 else 0

            head_depth_px = get_head_depth_from_side_image(side_image_path)
            head_length_cm = head_depth_px * pixel_to_cm_ratio
            if head_length_cm < 10: 
                print("Warning: Side image processing failed. Falling back to forehead-chin measurement for depth.")
                forehead_top = face_landmarks.landmark[TOP_OF_FOREHEAD_INDEX]
                chin_bottom = face_landmarks.landmark[BOTTOM_OF_CHIN_INDEX]
                head_length_px_fallback = calculate_pixel_distance(forehead_top, chin_bottom, image_w, image_h)
                head_length_cm = head_length_px_fallback * pixel_to_cm_ratio

            head_width_cm = face_width_px * pixel_to_cm_ratio

            forehead_top = face_landmarks.landmark[TOP_OF_FOREHEAD_INDEX]
            chin_bottom = face_landmarks.landmark[BOTTOM_OF_CHIN_INDEX]
            head_height_px = calculate_pixel_distance(forehead_top, chin_bottom, image_w, image_h)
            ...
            return {
                ...
                'eye_to_eye': corrected_assumed_ipd_cm,
                'head_height': head_height_cm,
                'head_circumference_A': (head_width_cm + head_length_cm) * 1.57, 
        ...
        print(f"ERROR in get_dynamic_2d_measurements for {front_image_path}: {e}")
        return {'head_width': 15.0, 'head_length': 20.0, 'ear_to_ear': 15.0, 'eye_to_eye': ASSUMED_IPD_CM, 'head_height': 20.0, 'head_circumference_A': 55.0, 'forehead_to_back_B': 20.0, 'cross_measurement_C': 15.0, 'under_chin_D': 10.0, 'eyebrow_to_earlobe_E': 8.0, 'eye_corner_to_ear_F': 7.0, 'ear_height_G': 6.5, 'ear_width_H': 3.5, 'cheek_guard_clearance_L': 2.0, 'cheek_guard_height_M': 5.0, 'cheek_guard_width_N': 4.0}
...
def get_surface_measurements_from_model(model_path: str, gender: str, front_image_path: str, side_image_path: str) -> Optional[Dict[str, float]]:
    ...
    try:
        ...
        measurements_2d = get_dynamic_2d_measurements(front_image_path, side_image_path)
        ...
        measurements = get_dynamic_2d_measurements(front_image_path, side_image_path)
