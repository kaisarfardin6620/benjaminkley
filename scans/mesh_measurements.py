...
def _find_anatomical_landmarks(mesh):
    ...
    print(f"--- Anatomical landmarks identified (vertex indices) ---")
    return landmarks
...
def _calculate_surface_distance(mesh, start_idx, end_idx):
    try:
        ...
        print(f"WARNING: Could not find a surface path between vertices {start_idx} and {end_idx}.")
        return 0.0
...
def perform_all_measurements(mesh):
    ...
    print(f"--- Final Measurements (cm): {measurements} ---")
    return measurements